...
function computeDistance(sat1, sat2) {
  ...
  const x1 =
    (R + sat1.alt_km) * Math.cos(toRad(sat1.lat)) * Math.cos(toRad(sat1.lng));
  const y1 =
    (R + sat1.alt_km) * Math.cos(toRad(sat1.lat)) * Math.sin(toRad(sat1.lng));
  const z1 = (R + sat1.alt_km) * Math.sin(toRad(sat1.lat));

  const x2 =
    (R + sat2.alt_km) * Math.cos(toRad(sat2.lat)) * Math.cos(toRad(sat2.lng));
  const y2 =
    (R + sat2.alt_km) * Math.cos(toRad(sat2.lat)) * Math.sin(toRad(sat2.lng));
...
function getRiskLevel(distance_km) {
  if (distance_km <= 20) return "HIGH";
  if (distance_km <= 150) return "MEDIUM";
  if (distance_km <= 400) return "LOW";
  return null;
}
...
function detectCollisions(objects) {
  ...
  const ALTITUDE_BAND = 400; // only check 2 objects for collision, if they are within 400 km of each other in altitude.
  ...
  for (let i = 0; i < objects.length; i++) {
    for (let j = i + 1; j < objects.length; j++) {
      ...
      if (Math.abs(objects[i].alt_km - objects[j].alt_km) > ALTITUDE_BAND)
        continue;
      ...
      if (risk) {
        // this only runs if the risk is HIGH, MEDIUM or LOW.
