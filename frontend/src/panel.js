...
function startLiveRefresh(id) {
  _liveRefresh = setInterval(() => {
    ...
    if (window.selectedSatellite?.id === id) Object.assign(window.selectedSatellite, fresh);
    pushHistory(fresh);
...
function stopLiveRefresh() {
  if (_liveRefresh) { clearInterval(_liveRefresh); _liveRefresh = null; }
}
...
function renderSingle(sat) {
  const cfg    = TYPE_CONFIG[sat.type] || { color: '#aaa', label: sat.type?.toUpperCase() || 'UNKNOWN' };
  const r      = 6371 + sat.alt_km;
  const period = ((2 * Math.PI * Math.pow(r, 1.5)) / Math.sqrt(398600.4418) / 60).toFixed(1);
  const orbDay = (1440 / period).toFixed(2);

  if (!_history[sat.id]) {
    _history[sat.id] = Array.from({ length: HISTORY_LEN }, () => sat.alt_km + (Math.random() - 0.5) * 4);
  }
  pushHistory(sat);
...
function setupRemoveBtn(sat) {
  ...
  let confirmed = false, timer = null;
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      window.satelliteMap = {};
      (result.satellites || []).forEach(s => { window.satelliteMap[s.id] = s; });
      showImpactModal(result.impact, result.removed?.name || sat.name);
      window.dispatchEvent(new CustomEvent('satellites-updated', { detail: result.satellites || [] }));
      window.dispatchEvent(new CustomEvent('collisions-updated', { detail: result.collisions || [] }));
...
function showImpactModal(impact, satName) {
  ...
  const kh = e => { if (e.key === 'Escape' || e.key.toLowerCase() === 'f') { close(); document.removeEventListener('keydown', kh); } };
...
function spawnParticles() {
  ...
  for (let i = 0; i < 18; i++) {
    ...
    p.style.cssText = `--sx:${sx};--sy:${sy};--ex:${ex};--ey:${ey};--dur:${dur};left:${sx};top:${sy};background:${Math.random()>.5?'#00ffaa':'#00c8ff'}`;
    c.appendChild(p);
  }
...
export function initAddSatelliteForm() {
  ...
  document.getElementById('add-satellite-form').addEventListener('submit', async e => {
    e.preventDefault();
    const tle1 = input1.value.trim();
    const tle2 = input2.value.trim();
    if (!tle1.startsWith('1 ')) { showStatus(statusEl, 'error', 'TLE Line 1 must start with "1 "'); input1.classList.add('invalid'); return; }
    if (!tle2.startsWith('2 ')) { showStatus(statusEl, 'error', 'TLE Line 2 must start with "2 "'); input2.classList.add('invalid'); return; }

    btn.classList.add('loading');
    btn.disabled = true;
    showStatus(statusEl, 'loading', 'Injecting — running conjunction analysis...');
    resetAlertHistory();

    try {
      const result = await addSatellite(tle1, tle2);
      window.dispatchEvent(new CustomEvent('satellites-updated',  { detail: result.satellites }));
      window.dispatchEvent(new CustomEvent('satellite-injected',  { detail: result.added }));

      if (result.new_collisions?.length > 0) {
        const highs = result.new_collisions.filter(c => c.risk === 'HIGH').length;
        showStatus(statusEl, 'warning', `${result.added.name} — ${result.new_collisions.length} conjunction(s) detected${highs ? ` (${highs} HIGH RISK)` : ''}`);
        renderCollisionAlerts(result.new_collisions);
        showToast(`${result.new_collisions.length} new collision risk(s) detected`, 'warning');
      } else {
        showStatus(statusEl, 'success', `${result.added.name} — no collision risks found`);
...
function validateLine(el, startChar) {
  ...
  if (!val) { el.className = 'f-input'; return; }
  el.className = 'f-input ' + (val.startsWith(startChar + ' ') ? 'valid' : 'invalid');
}

async function typewriter(el, text, speed = 16) {
  el.value = '';
  el.className = 'f-input';
  for (const ch of text) { el.value += ch; await new Promise(r => setTimeout(r, speed)); }
}
...
function showStatus(el, type, msg) {
  ...
  el.textContent = msg;
  if (type === 'success') setTimeout(() => { el.style.display = 'none'; }, 5500);
}
